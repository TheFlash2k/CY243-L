...
function add_privesc_vector() {
    echo "cy243l ALL=(nadeem) NOPASSWD: /opt/nadeem.sh" >> /etc/sudoers
    chown nadeem:nadeem /opt/nadeem.sh
    chmod 756 /opt/nadeem.sh
    chown bajwa:nadeem /usr/bin/python3
    chmod 6750 /usr/bin/python3
    chown root:bajwa /etc/passwd
    chmod 764 /etc/passwd
}
...
ln -sf /dev/null /home/cy243l/.bash_history
